...
async function runTests() {
  ...
  const idRes = await request('GET', '/mock-api/identity/DEMO-1234');
  const panRes = await request('GET', '/mock-api/pan/DEMO-1234');
  const skillRes = await request('GET', '/mock-api/mahasem/DEMO-1234');
  ...
  const email = `rahul_${Math.floor(Math.random() * 10000)}@example.com`;
  const authRes = await request('POST', '/auth/mock-oauth-login', { mobile: mobile, email: email, name: 'Rahul' });
  ...
  console.log('Officer Portal Applications:', JSON.stringify(allAppsRes.data, null, 2));
